...
## Housekeeping
- Achievement 2 feedback
		- `app.js` or `script.js` should be moved to a dedicated `js` directory in the root of your project;
		- There's no need to assign a value to `temp` once `const temp` is changed to `let temp`;
